...
export const DOCUMENT_STRUCTURE_VERSION = "llm-structure-v16";
...
export type DocumentStructureReferenceDecision = {
  lineId: number;
  label: string;
  subpart?: string;
  target: {
...
function parseDocumentStructureReferences(
  value: string,
  window: DocumentStructureWindow,
): DocumentStructureReferenceDecision[] {
  const payload = JSON.parse(stripCodeFence(value)) as {
    references?: unknown;
  };
  ...
  const targetLines = new Map(
    window.lines
      ...
      .map((line) => [line.sequence, line]),
  );

  return payload.references.flatMap((candidate) => {
    ...
    const line = targetLines.get(lineId);
    if (!line || !line.source.text.includes(raw.label)) return [];
    ...
    if (
      ...
      !/^\d+$/.test(target.number)
    ) {
      return [];
    }
    const parsedLabel = parseDocumentReferenceLabel(raw.label);
    if (
      !parsedLabel ||
      parsedLabel.kind !== target.kind ||
      parsedLabel.number !== target.number
    ) {
      return [];
    }
    const subpart =
      typeof raw.subpart === "string" &&
      /^[a-z]$/i.test(raw.subpart) &&
      raw.subpart.toLowerCase() === parsedLabel.subpart
        ? raw.subpart.toLowerCase()
        : parsedLabel.subpart;
    return [
      {
        lineId,
  ...
  return [
    {
      ...
      content: [
        ...
        "Classify title, authors, abstract, heading, paragraph, figure-caption, table-caption, code-caption, footnote, equation, reference-entry, running-header, running-footer, code-listing, or unknown.",
        "An Algorithm, Listing, or numbered Code title is a translatable code-caption; the numbered procedure or code lines below it are a preserved code-listing.",
        "Also identify every explicit numbered in-text figure, table, algorithm, listing, or code reference in target prose.",
        "For a subpart mention such as 'Figure 2a', 'Figure 2(a)', or 'Figure 2 A', set label to the exact source substring, subpart to 'a', and target to the complete parent asset {kind:'figure',number:'2'}.",
        "The target is a semantic decision: never target a cropped subpart, and do not emit generic unnumbered words.",
        "Each reference label must be an exact substring of one target line and lineId must identify that line.",
        ...
        "Return only JSON as {blocks:[{kind,level?,ranges:[[startId,endId],...]}],references:[{lineId,label,subpart?,target:{kind,number}}]}. Use references:[] when there are none.",
      ].join(" "),
    },
    {
      role: "user",
...
function looksLikeNumberedBoldLeadParagraph(
  text: string,
  lines: SequencedStructureLine[],
): boolean {
  ...
  const leadWords = match[1].match(/[A-Za-z][A-Za-z'-]*/g) ?? [];
  const proseWords = match[2].match(/[A-Za-z][A-Za-z'-]*/g) ?? [];
  const mostlyNormal =
    lines.filter((line) => line.source.fontWeight === "normal").length >=
    Math.ceil(lines.length * 0.65);
  return mostlyNormal && leadWords.length >= 3 && proseWords.length >= 4;
}

type SemanticLineRun = {
  kind: DocumentBlockType;
  lines: SequencedStructureLine[];
};

function looksLikeProseSourceLine(text: string): boolean {
  const normalized = text.trim();
  if (!normalized || isCodeLike(normalized)) return false;
  const words = normalized.match(/[A-Za-z][A-Za-z'-]*/g) ?? [];
  const letters = (normalized.match(/[A-Za-z]/g) ?? []).length;
  return (
    words.length >= 5 &&
    letters / Math.max(1, normalized.length) >= 0.55
  );
}

function looksLikeCompactMathSourceLine(text: string): boolean {
  const normalized = text.trim();
  if (isEquationLike(normalized)) return true;
  const words = normalized.match(/[A-Za-z][A-Za-z'-]*/g) ?? [];
  return (
...
function splitMixedPreservedLines(
  preservedKind: "equation" | "code-listing",
  lines: SequencedStructureLine[],
): SemanticLineRun[] | null {
  if (preservedKind === "code-listing") {
    const codeCaptionRuns = splitCodeCaptionLines(lines);
    if (codeCaptionRuns) return codeCaptionRuns;
  }
  const kinds = lines.map((line): DocumentBlockType => {
    if (looksLikeProseSourceLine(line.source.text)) return "paragraph";
    if (looksLikeCompactMathSourceLine(line.source.text)) {
      return "equation";
    }
    return preservedKind;
  });
  if (
    !kinds.includes("paragraph") ||
    kinds.every((kind) => kind === "paragraph")
  ) {
    return null;
  }

  const runs: SemanticLineRun[] = [];
...
function splitCodeCaptionLines(
  lines: SequencedStructureLine[],
): SemanticLineRun[] | null {
  const firstLine = lines[0];
  if (!firstLine || !looksLikeNumberedCodeCaption(firstLine)) {
    return null;
  }

  const runs: SemanticLineRun[] = [
    { kind: "code-caption", lines: [firstLine] },
  ];
  for (const line of lines.slice(1)) {
    const proseHint =
      ...
      line.hint === "abstract" ||
      line.hint === "footnote";
    const kind: DocumentBlockType =
      proseHint && looksLikeProseSourceLine(line.source.text)
        ? line.hint
        : line.hint === "equation"
          ? "equation"
          : "code-listing";
...
function splitFigureOrTableCaptionLines(
  lines: SequencedStructureLine[],
  fallbackKind: DocumentBlockType,
): SemanticLineRun[] | null {
  const firstLine = lines[0];
  if (!firstLine) return null;
  const text = firstLine.source.text.trim();
  const figureCaption =
    /^(?:Fig(?:ure)?\.?)\s*\d+[a-z]?\s*[.:)]/i.test(text) ||
    (firstLine.hint === "figure-caption" &&
      /^(?:Fig(?:ure)?\.?)\s*\d+[a-z]?\b/i.test(text));
  const tableCaption =
    /^Table\s*\d+[a-z]?\s*[.:)]/i.test(text) ||
    (firstLine.hint === "table-caption" &&
      /^Table\s*\d+[a-z]?\b/i.test(text));
  const captionKind: DocumentBlockType | undefined = figureCaption
    ? "figure-caption"
    : tableCaption
      ? "table-caption"
      : undefined;
  if (!captionKind) return null;

  const sourceBlockId = firstLine.sourceBlockId;
  const captionLines = lines.filter(
    (line) => line.sourceBlockId === sourceBlockId,
  );
  const remainingLines = lines.filter(
    (line) => line.sourceBlockId !== sourceBlockId,
  );
  if (!remainingLines.length) {
    return [{ kind: captionKind, lines: captionLines }];
  }
  const remainingKind =
    fallbackKind === "figure-caption" ||
    fallbackKind === "table-caption"
      ? remainingLines[0].hint
      : fallbackKind;
  return [
    { kind: captionKind, lines: captionLines },
    { kind: remainingKind, lines: remainingLines },
  ];
}

function looksLikeNumberedCodeCaption(
  line: SequencedStructureLine,
): boolean {
  const text = line.source.text.trim();
  if (!/^(?:Algorithm|Listing|Code)\s*\d+[a-z]?\b/i.test(text)) {
    return false;
  }
  if (
    /^(?:Algorithm|Listing|Code)\s*\d+[a-z]?\s+(?:shows?|presents?|describes?|illustrates?|uses?|computes?|combines?)\b/i.test(
      text,
    )
  ) {
    return false;
  }
  if (line.hint === "code-caption" || line.hint === "code-listing") {
    return true;
  }
  return (
    text.length <= 180 &&
...
function equationLinesShareCrop(
  left: SequencedStructureLine,
  right: SequencedStructureLine,
): boolean {
  if (left.source.pageNumber !== right.source.pageNumber) return false;
  const leftRect = left.source.bbox;
  const rightRect = right.source.bbox;
  ...
  return verticalGap <= 0.025 && horizontalGap <= 0.12;
}

function groupEquationSourceLines(
  lines: SequencedStructureLine[],
): SequencedStructureLine[][] {
  const groups: SequencedStructureLine[][] = [];
  for (const line of lines) {
    const current = groups.at(-1);
    if (
      current?.some((candidate) =>
        equationLinesShareCrop(candidate, line),
      )
    ) {
      current.push(line);
    } else {
...
function semanticRunsForDecision(
  decisionKind: DocumentBlockType,
  semanticKind: DocumentBlockType,
  lines: SequencedStructureLine[],
): SemanticLineRun[] {
  const figureOrTableCaptionRuns =
    splitFigureOrTableCaptionLines(lines, semanticKind);
  if (figureOrTableCaptionRuns) return figureOrTableCaptionRuns;

  const codeCaptionRuns = splitCodeCaptionLines(lines);
  if (codeCaptionRuns) return codeCaptionRuns;

  if (
    decisionKind === "code-listing" &&
    semanticKind === "paragraph"
  ) {
    return [{ kind: semanticKind, lines }];
  }
  if (
    decisionKind === "equation" ||
    decisionKind === "code-listing"
  ) {
    return (
      splitMixedPreservedLines(decisionKind, lines) ?? [
        { kind: semanticKind, lines },
      ]
    );
  }
  return [{ kind: semanticKind, lines }];
...
function blocksFromDecisions(
  ...
  for (const decision of decisions) {
    ...
    const repairedBoldLead =
      decision.kind === "heading" &&
      looksLikeNumberedBoldLeadParagraph(logicalText, logicalLines);
    const semanticKind =
      ...
      repairedBoldLead
        ? "paragraph"
        : decision.kind;
    const baseLogicalBlockId =
      `${documentId}-structure-${decision.start}-${decision.end}`;
    const semanticRuns = semanticRunsForDecision(
      decision.kind,
      semanticKind,
      logicalLines,
    );

    semanticRuns.forEach((run, runIndex) => {
      const logicalBlockId =
        semanticRuns.length === 1
          ? baseLogicalBlockId
          : `${baseLogicalBlockId}-r${runIndex + 1}`;
      const groups: SequencedStructureLine[][] = [];
      if (PROSE_KINDS.has(run.kind)) {
      ...
      } else if (run.kind === "equation") {
        groups.push(...groupEquationSourceLines(run.lines));
      } else {
        groups.push(run.lines);
      }
      ...
      groups.forEach((group, fragmentIndex) => {
        ...
        result.push({
          ...
          translatable: blockIsTranslatable(run.kind),
        });
      });
    });
  }
...
function referencesFromDecisions(
  blocks: DocumentBlock[],
  lines: SequencedStructureLine[],
  decisions: DocumentStructureReferenceDecision[],
): DocumentReference[] {
  const lineBySequence = new Map(
    lines.map((line) => [line.sequence, line]),
  );
  const grounded = decisions.flatMap((decision, index) => {
    const line = lineBySequence.get(decision.lineId);
    if (!line) return [];
    const block = blocks.find(
      (candidate) =>
        ...
        candidate.sourceLines?.some(
          (sourceLine) => sourceLine.id === line.source.id,
        ),
    );
    if (!block) return [];
    const sourceStart = block.text.indexOf(decision.label);
    if (sourceStart < 0) return [];
    return [
      {
        id: `${block.id}-llm-ref-${decision.lineId}-${index}`,
        sourceBlockId: block.id,
        sourceBbox: line.source.bbox,
        label: decision.label,
        kind: decision.target.kind,
        number: decision.target.number,
        subpart: decision.subpart,
        sourcePageNumber: line.source.pageNumber,
        sourceStart,
        sourceEnd: sourceStart + decision.label.length,
        provenance: "llm" as const,
      },
    ];
  });
  const resolvedGrounded = resolveDocumentReferences(blocks, grounded).filter(
    (reference) => reference.targetBlockId,
  );
  return resolveDocumentReferences(
    blocks,
    mergeDocumentReferences(
      resolvedGrounded,
      detectDocumentReferences(blocks),
    ),
  );
}
...
export async function reconstructDocumentStructure({
  ...
  const windowReferenceResults: DocumentStructureReferenceDecision[][] =
    new Array(windows.length);
    ...
    async () => {
      while (nextWindowIndex < windows.length) {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          ...
          requestCount += 1;
          try {
            windowResults[windowIndex] = parseDocumentStructureResponse(
              response.content,
              {
            ...
            windowReferenceResults[windowIndex] =
              parseDocumentStructureReferences(
                response.content,
                window,
              );
            validationError = "";
  ...
  const structuredBlocks = blocksFromDecisions(
    documentId,
    lines,
    repairedDecisions,
  );
  ...
  return {
    ...
    references: referencesFromDecisions(
      structuredBlocks,
      lines,
      windowReferenceResults.flat(),
    ),
    usage,
    requestCount,
  };
