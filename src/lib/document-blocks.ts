...
function mergeInlineScriptLines(
  lines: TextLine[],
  pageWidth: number,
): TextLine[] {
  const scripts = lines
    .filter(
      (line) =>
        line.text.length <= 24 &&
        line.maxX - line.minX <= pageWidth * 0.12,
    )
    .sort((left, right) => left.fontSize - right.fontSize);
  const removed = new Set<TextLine>();

  for (const script of scripts) {
    if (removed.has(script)) continue;
    const target = lines
      .filter(
        (candidate) =>
          candidate !== script &&
          !removed.has(candidate) &&
          script.fontSize <= candidate.fontSize * 0.82 &&
          Math.abs(candidate.items[0].y - script.items[0].y) <=
            candidate.fontSize * 0.8 &&
          horizontalGap(candidate, script.items[0]) <=
            Math.max(
              8,
              candidate.fontSize * 1.8,
              pageWidth * 0.018,
            ),
      )
      .sort(
        (left, right) =>
          Math.abs(left.items[0].y - script.items[0].y) -
            Math.abs(right.items[0].y - script.items[0].y) ||
          horizontalGap(left, script.items[0]) -
            horizontalGap(right, script.items[0]),
      )[0];
    if (!target) continue;

    target.items.push(...script.items);
    target.text = joinLineItems(target.items);
    target.minX = Math.min(target.minX, script.minX);
    target.minY = Math.min(target.minY, script.minY);
    target.maxX = Math.max(target.maxX, script.maxX);
    target.maxY = Math.max(target.maxY, script.maxY);
    removed.add(script);
  }

  return lines.filter((line) => !removed.has(line));
}
...
function makeLines(items: RawTextItem[], pageWidth: number): TextLine[] {
  ...
  return mergeInlineScriptLines(lines, pageWidth).sort(
...
function isRotatedSideText(
  transform: number[],
  pageWidth: number,
): boolean {
  const rotated =
    Math.abs(transform[1] ?? 0) > Math.abs(transform[0] ?? 0);
  const x = transform[4] ?? 0;
  return rotated && (x <= pageWidth * 0.09 || x >= pageWidth * 0.91);
}
...
function classifyBlock(
  ...
  const explicitTableCaption =
    /^(?:table|표)\s*\d+[a-z]?\s*[.:)]/i.test(normalized);
  const compactTableLabel =
    /^(?:table|표)\s*\d+[a-z]?(?:\s|$)/i.test(normalized);
  const explicitCodeCaption =
    /^(?:Algorithm|Listing|Code|알고리즘|목록|코드)\s*\d+[a-z]?(?:\s|$)/i.test(
      normalized,
    );
  ...
  if (explicitCodeCaption) return "code-caption";
...
export function groupPageTextItems(
  ...
  return groups.map(({ lines: group }, readingOrder) => {
    ...
    return {
      id: `${context.documentId}-p${context.pageNumber}-v13-b${readingOrder}`,
...
export async function extractDocumentBlocks(
  ...
  const extractPage = async (pageNumber: number) => {
    ...
    const items: RawTextItem[] = content.items.flatMap((item) => {
      ...
      if (isRotatedSideText(transform, viewport.width)) return [];
