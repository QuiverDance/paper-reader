...
import { findDocumentReferenceLabels } from "./document-references";
export {
  groupTranslationParagraphs,
  type TranslationParagraph,
} from "./translation-paragraphs";
...
export function protectCitations(
  ...
  let protectedReferences = protectedMath;
  const protectedReferenceLabels = findDocumentReferenceLabels(
    protectedMath,
  ).map((reference) => {
    const token = `⟦${tokenNamespace}REFERENCE_${markers.length + 1}⟧`;
    markers.push({ token, source: reference.label });
    return { reference, token };
  });
  for (const { reference, token } of protectedReferenceLabels.reverse()) {
    protectedReferences =
      protectedReferences.slice(0, reference.start) +
      token +
      protectedReferences.slice(reference.end);
  }
  return { text: protectedReferences, markers };
}
