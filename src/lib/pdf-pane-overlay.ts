...
export type OverlayPage<T> = {
  pageNumber: number;
  host: T;
  revision: number;
};

type ReferenceTokenCandidate = {
...
export function chooseReferenceTokenRects(
  candidates: ReferenceTokenCandidate[],
  nativeLinkRects: NormalizedRect[],
): NormalizedRect[] {
  const ranked = candidates.map((candidate) => ({
    ...candidate,
    nativeOverlap: candidate.rects.reduce(
      (total, rect) =>
        total +
        nativeLinkRects.reduce(
          (sum, nativeRect) =>
            sum + intersectionArea(rect, nativeRect),
          0,
        ),
      0,
    ),
  ...
  ranked.sort(
    (left, right) =>
      Number(right.nativeOverlap > 0) -
        Number(left.nativeOverlap > 0) ||
      right.nativeOverlap - left.nativeOverlap ||
      left.distance - right.distance,
  );
  return mergeReferenceRects(ranked[0]?.rects ?? []);
}

export function reconcileOverlayPages<T>(
  current: OverlayPage<T>[],
  discovered: ReadonlyArray<{ pageNumber: number; host: T }>,
  revisedPageNumber?: number,
): OverlayPage<T>[] {
  const currentByPage = new Map(
    current.map((page) => [page.pageNumber, page]),
  );
  let changed = current.length !== discovered.length;
  const next = discovered.map(({ pageNumber, host }) => {
    const previous = currentByPage.get(pageNumber);
    const revision =
      previous && revisedPageNumber === pageNumber
        ? previous.revision + 1
        : previous?.revision ?? 0;
    if (
      previous &&
...
export function referenceRectStyle(
  ...
  const left = `${rotated.x * 100}%`;
  const top = `${rotated.y * 100}%`;
  const width = `${rotated.width * 100}%`;
  const height = `${rotated.height * 100}%`;
  if (!padding) {
  ...
  return {
    ...
    height: `calc(${height} + ${padding * 2}px)`,
  };
}
...
export function locateReferenceTokenRects(
  ...
  const nativeLinkRects = Array.from(
    pageElement.querySelectorAll<HTMLElement>(
      ".annotationLayer a[href], .annotationLayer a.internalLink",
    ),
  )
  ...
  const candidates: ReferenceTokenCandidate[] = [];
  ...
  if (candidates.length) {
    return chooseReferenceTokenRects(candidates, nativeLinkRects);
  }
  const linksInSourceBlock = nativeLinkRects.filter(
    (rect) => intersectionArea(rect, expected) > 0,
  );
  return linksInSourceBlock.length === 1
    ? linksInSourceBlock
    : [];
}
