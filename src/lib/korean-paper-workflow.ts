...
function warningSummary(warnings: RetypesetWarning[]): string {
  return warnings
    .slice(0, 5)
...
export async function applyKoreanPaperCandidate(
  ...
  try {
    ...
    if (validationBlockers.length) {
      throw new Error(
        `자동 무결성 검사에서 ${validationBlockers.length}개 오류를 발견했습니다. 기존 한국어 논문은 유지됩니다. Blockers: ${warningSummary(validationBlockers)}`,
      );
    }
    ...
    if (renderBlockers.length) {
      throw new Error(
        `PDF 생성 검사에서 ${renderBlockers.length}개 오류를 발견했습니다. 기존 한국어 논문은 유지됩니다. Blockers: ${warningSummary(renderBlockers)}`,
      );
    }
