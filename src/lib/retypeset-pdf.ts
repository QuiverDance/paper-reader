...
export function inferCaptionFontSize(bodySize: number): number {
  return bodySize * 0.95;
}
...
export function registeredEquationPadding(rect: NormalizedRect): number {
  return rect.width <= 0.16 && rect.height <= 0.04 ? 0.75 : 4;
}
...
export function shouldRegisterPreservedFragment(
  ...
  if (block.type === "code-listing") {
    return !looksLikeNaturalProse(block.text);
  }
...
function academicWrapUnits(paragraph: string): string[] {
  const words = [...paragraph.matchAll(/\S+/g)].map((match) => ({
    text: match[0],
    start: match.index,
    end: match.index + match[0].length,
  }));
  if (words.length < 2) return words.map((word) => word.text);

  const joinedThrough = new Map<number, number>();
  for (const reference of findDocumentReferenceLabels(paragraph)) {
    const firstWord = words.findIndex((word) => word.end > reference.start);
    if (firstWord < 0) continue;
    let lastWord = firstWord;
    while (
      lastWord + 1 < words.length &&
      words[lastWord + 1].start < reference.end
    ) {
      lastWord += 1;
    }
    if (lastWord > firstWord) {
      joinedThrough.set(
        firstWord,
        Math.max(joinedThrough.get(firstWord) ?? firstWord, lastWord),
      );
    }
  }
  ...
  for (let index = 0; index < words.length; index += 1) {
    const lastWord = joinedThrough.get(index) ?? index;
    units.push(
      paragraph
        .slice(words[index].start, words[lastWord].end)
        .replace(/\s+/g, " "),
    );
    index = lastWord;
  }
...
export function academicTextLines(
  ...
  for (const paragraph of paragraphs) {
    const words = academicWrapUnits(paragraph);
    if (!words.length) {
...
function blockText(
  ...
  if (translation?.status === "translated" && translation.translatedText.trim()) {
    return {
      text: localizeDocumentReferenceLabels(
...
function detectInlineLinks(
  ...
  for (const reference of findDocumentReferenceLabels(text)) {
    const start = offsetAt(reference.start);
    const end = offsetAt(reference.end);
    links.push({
      page,
      x: x + start,
      y: y - 1,
      ...
      height: lineHeight,
      key: documentReferenceKey(reference),
    });
  }
  for (const match of text.matchAll(/\[(\d+)\]/g)) {
    const start = offsetAt(match.index);
    const end = offsetAt(match.index + match[0].length);
    links.push({
      page,
      x: x + start,
      y: y - 1,
...
export async function createRetypesetPdf(
  ...
  const drawRegisteredFragment = async (
    pageNumber: number,
    rect: NormalizedRect,
    kind: "asset" | "equation" | "fragment" = "fragment",
    reserve = true,
    assetKind?: PaperAsset["kind"],
    ...
    const crop =
      kind === "equation"
        ? equationSourceCrop(
            ...
            size.width,
            size.height,
            registeredEquationPadding(rect),
          )
        : kind === "asset"
          ? sourceRegisteredAssetCrop(
              rect,
    ...
    if (reserve) reserveSlot(pageNumber, rect);
    return true;
  };
        ...
        message: `${
          asset.kind === "figure"
            ...
            : asset.kind === "table"
              ? "표"
              : "코드"
        } ${asset.number}의 벡터 영역을 가져오지 못했습니다.`,
      });
  ...
  if (frontMatterRect) {
    frontMatterRegistered = await drawRegisteredFragment(
      paper.contentStartPage,
      frontMatterRect,
    );
    if (
  ...
  const registeredAssetByCaptionId = new Map(
    paper.assets
      .filter((asset) => registeredAssetIds.has(asset.id))
      .map((asset) => [asset.captionBlockId, asset]),
  );
  ...
  for (const block of blocks) {
    ...
    const size = inferCaptionFontSize(bodySize);
    ...
    const registeredAsset = registeredAssetByCaptionId.get(block.id);
    if (registeredAsset) {
      const slot = createRegisteredAssetSlot(
        registeredAsset,
        captionRect,
        {
          pageTop: PAGE_TOP_RATIO,
          pageWidth,
          pageHeight,
        },
      );
      reserveSlot(slot.pageNumber, slot.exclusionRect);
    } else {
      reserveSlot(block.pageNumber, captionRect);
    }
  ...
  for (const block of ordered) {
    ...
    const isCaption =
      ...
      block.type === "code-caption";
    if (
      (asset?.kind === "table" || asset?.kind === "code") &&
    ...
    const boldFont = rendered.translated
      ? paper.bodyFontStyle === "sans"
        ...
        : fonts.koreanBodyBold
      : fonts.latinBold;
    const size = isCaption
      ? inferCaptionFontSize(bodySize)
      : block.type === "footnote"
        ? bodySize * 0.86
        : bodySize;
    drawLines(rendered.text, {
