...
describe("section translation protection", () => {
  ...
  it("round-trips parent and subpart asset references exactly", () => {
    const source =
      "Figure 5 A illustrates yielding, Figure 5(B) shows combining, and Table 1 compares them.";
    const protectedText = protectCitations(source);
    const referenceMarkers = protectedText.markers.filter(({ token }) =>
      token.includes("REFERENCE"),
    );

    expect(referenceMarkers).toHaveLength(3);
    expect(protectedText.text).not.toContain("Figure 5 A");
    expect(
      restoreProtectedText(
        `${referenceMarkers[0].token}는 양보를, ${referenceMarkers[1].token}는 결합을 보여주며 ${referenceMarkers[2].token}은 둘을 비교한다.`,
        referenceMarkers,
      ),
    ).toBe(
      "Figure 5 A는 양보를, Figure 5(B)는 결합을 보여주며 Table 1은 둘을 비교한다.",
    );
  });
