...
describe("registered source fragments", () => {
  ...
  it("does not preserve collapsed PDF prose mislabeled as a code listing", () => {
    const collapsedProse = block(
      "collapsed-prose",
      "code-listing",
      "andwhencacheloadingstallsareunavoidable,thescheduler",
    );

    expect(
      shouldRegisterPreservedFragment(collapsedProse, new Set()),
    ).toBe(false);
  });

  it("does not preserve prose merged with a chart legend", () => {
    const mergedLegendAndProse = block(
      "merged-legend-prose",
      "code-listing",
    ...
    expect(
      shouldRegisterPreservedFragment(mergedLegendAndProse, new Set()),
    ).toBe(false);
    expect(shouldOmitMixedSourceFragment(mergedLegendAndProse)).toBe(true);
  });
