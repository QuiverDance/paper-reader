...
describe("document references", () => {
  it("keeps source and translated reference tokens on their own viewer surface", () => {
    const blocks: DocumentBlock[] = [
      {
        id: "mixed-reference-layer",
        documentId: "doc",
        pageNumber: 8,
        type: "paragraph",
        text:
          "The procedure is detailed in Algorithm 1. 절차는 알고리즘 1에 상세히 제시되어 있다.",
    ...
    expect(
      detectDocumentReferences(blocks, { language: "source" }).map(
        (reference) => reference.label,
      ),
    ).toEqual(["Algorithm 1"]);
    expect(
      detectDocumentReferences(blocks, { language: "translation" }).map(
        (reference) => reference.label,
      ),
    ).toEqual(["알고리즘 1"]);
  });
